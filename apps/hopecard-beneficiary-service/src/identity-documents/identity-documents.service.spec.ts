...
describe('IdentityDocumentsService', () => {
  ...
  describe('uploadDocument', () => {
    it('throws BadRequestException when no file', async () => {
      await expect(
        service.uploadDocument('uid-1', null as unknown as Express.Multer.File),
    ...
    it('throws BadRequestException when file is too large', async () => {
      ...
      await expect(
        service.uploadDocument('uid-1', file),
      ).rejects.toBeInstanceOf(BadRequestException);
    });
    ...
    it('uploads document and emits event', async () => {
      ...
      mockStorageUpload.mockResolvedValueOnce({
        data: { path: 'p-1/123-document.jpg' },
        error: null,
      });
      mockStorageGetPublicUrl.mockReturnValue({
        data: { publicUrl: 'https://cdn.example.com/doc.jpg' },
      });
    ...
    it('handles png and pdf extensions', async () => {
      for (const ext of ['png', 'pdf']) {
        ...
        mockStorageUpload.mockResolvedValueOnce({
          data: { path: `p-1/file.${ext}` },
          error: null,
        });
        mockStorageGetPublicUrl.mockReturnValue({
          data: { publicUrl: 'https://cdn/file' },
        });
  ...
  describe('getDocuments', () => {
    ...
    it('throws BadRequestException on DB error', async () => {
      ...
      await expect(service.getDocuments('uid-1')).rejects.toBeInstanceOf(
        BadRequestException,
      );
    });
    ...
    it('throws NotFoundException when profile not found', async () => {
      ...
      await expect(service.getDocuments('uid-x')).rejects.toBeInstanceOf(
        NotFoundException,
      );
    });
  });
  ...
  describe('deleteDocument', () => {
    it('deletes document and emits event', async () => {
      mockSingle
        ...
        .mockResolvedValueOnce({
          data: {
            id: 'doc-1',
            document_key: 'p-1/doc.jpg',
            status: 'pending',
            beneficiary_profile_id: 'p-1',
          },
          error: null,
        }); // fetch doc
    ...
    it('throws NotFoundException when document not found', async () => {
      ...
      await expect(
        service.deleteDocument('uid-1', 'doc-x'),
      ).rejects.toBeInstanceOf(NotFoundException);
    });
    ...
    it('throws ForbiddenException when document belongs to another profile', async () => {
      mockSingle
        ...
        .mockResolvedValueOnce({
          data: {
            id: 'doc-1',
            document_key: 'p-2/doc.jpg',
            status: 'pending',
            beneficiary_profile_id: 'p-2',
          },
          error: null,
        });

      await expect(
        service.deleteDocument('uid-1', 'doc-1'),
    ...
    it('throws NotFoundException when profile not found', async () => {
      ...
      await expect(
        service.deleteDocument('uid-x', 'doc-1'),
      ).rejects.toBeInstanceOf(NotFoundException);
    });
  });
  ...
  describe('getSignedUrl', () => {
    ...
    it('throws BadRequestException on storage error', async () => {
      ...
      await expect(
        service.getSignedUrl('uid-1', 'p-1/doc.jpg'),
      ).rejects.toBeInstanceOf(BadRequestException);
    });
    ...
    it('throws NotFoundException when profile not found', async () => {
      ...
      await expect(service.getSignedUrl('uid-x', 'key')).rejects.toBeInstanceOf(
        NotFoundException,
      );
    });
  });
