...
describe('BankAccountsService', () => {
  ...
  describe('getAccounts', () => {
    ...
    it('throws NotFoundException when profile not found', async () => {
      ...
      await expect(service.getAccounts('uid-x')).rejects.toBeInstanceOf(
        NotFoundException,
      );
    });
    ...
    it('throws BadRequestException on DB error fetching accounts', async () => {
      ...
      await expect(service.getAccounts('uid-1')).rejects.toBeInstanceOf(
        BadRequestException,
      );
    });
  });
  ...
  describe('updateAccount', () => {
    it('updates account and emits event', async () => {
      mockSingle
        .mockResolvedValueOnce({ data: { id: 'p-1' }, error: null }) // getProfile
        .mockResolvedValueOnce({
          data: { id: 'acc-1', bank_name: 'BPI' },
          error: null,
        }); // update result

      const result = await service.updateAccount('uid-1', 'acc-1', {
        bank_name: 'BPI',
      });
      expect(result.account).toBeDefined();
      expect(mockEmit).toHaveBeenCalledWith(
    ...
    it('throws NotFoundException when profile not found on update', async () => {
      ...
      await expect(
        service.updateAccount('uid-x', 'acc-1', {}),
      ).rejects.toBeInstanceOf(NotFoundException);
    });
    ...
    it('throws BadRequestException when update query fails', async () => {
      ...
      await expect(
        service.updateAccount('uid-1', 'acc-1', {}),
      ).rejects.toBeInstanceOf(BadRequestException);
    });
  });
  ...
  describe('deleteAccount', () => {
    ...
    it('throws NotFoundException when profile not found on delete', async () => {
      ...
      await expect(
        service.deleteAccount('uid-x', 'acc-1'),
      ).rejects.toBeInstanceOf(NotFoundException);
    });
    ...
    it('throws BadRequestException when delete update fails', async () => {
      ...
      await expect(
        service.deleteAccount('uid-1', 'acc-1'),
      ).rejects.toBeInstanceOf(BadRequestException);
    });
  });
});
