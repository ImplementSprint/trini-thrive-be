...
const SERVICES = [
  ...
  { name: 'notification',  port: process.env.HOPECARD_NOTIFICATION_PORT ?? '3105' },
];
...
describe('Hopecard — persona isolation (JWT claims)', () => {
  it('POST /api/v1/hopecard/donor/auth/login requires valid body', async () => {
    const res = await fetch(`http://localhost:3104/api/v1/hopecard/donor/auth/login`, {
      method: 'POST',
  ...
  it('GET /api/v1/hopecard/donor/cart without token returns 401', async () => {
    const res = await fetch(`http://localhost:3104/api/v1/hopecard/donor/cart?authUserId=test`).catch(() => null);
    if (!res) return;
    expect(res.status).toBe(401);
  });

  it('GET /api/v1/hopecard/admin/dashboard without token returns 401', async () => {
    const res = await fetch(`http://localhost:3101/api/v1/hopecard/admin/dashboard/metrics`).catch(() => null);
    if (!res) return;
    expect(res.status).toBe(401);
  });

  it('GET /api/v1/hopecard/beneficiary/bank-accounts without token returns 401', async () => {
    const res = await fetch(`http://localhost:3102/api/v1/hopecard/beneficiary/bank-accounts`).catch(() => null);
    if (!res) return;
    expect(res.status).toBe(401);
  });
});
